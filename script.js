...
// Sasaran & Modal Data per akaun
let accountGoalsData = {}; // Format: { "Main Account": { capital: 5000, monthlyGoal: 1008 } }
...
let currentDate = new Date(2026, 6, 27);
let currentYearForYearView = 2026;
let selectedDayStr = '2026-07-27';
...
function renderWeekView() {
    ...
    const days = [
        { name: '27 MON', dateStr: '2026-07-27' },
        { name: '28 TUE', dateStr: '2026-07-28' },
        { name: '29 WED', dateStr: '2026-07-29' },
        { name: '30 THU', dateStr: '2026-07-30' },
        { name: '31 FRI', dateStr: '2026-07-31' },
        { name: '1 SAT', dateStr: '2026-08-01' },
        { name: '2 SUN', dateStr: '2026-08-02' }
    ];

    let weekTotal = 0;
    days.forEach(d => {
        if (!showWeekends && (d.name.includes('SAT') || d.name.includes('SUN'))) return;
        const dTrades = accTrades.filter(t => t.date.startsWith(d.dateStr));
        let dPnL = 0;
        dTrades.forEach(t => { dPnL += (t.type === 'profit' ? t.amount : -t.amount); });
        weekTotal += dPnL;

        const [num, dayName] = d.name.split(' ');
        ...
        item.onclick = () => openIntradayView(d.dateStr);
        ...
        item.innerHTML = `
            <div class="flex items-center gap-3">
                <div class="text-center w-8">
                    <span class="text-base font-bold text-white block">${num}</span>
                    <span class="text-[10px] font-semibold text-gray-500 uppercase block">${dayName}</span>
                </div>
            </div>
...
function changeWeek(dir) { showToast('Pertukaran minggu'); }

// 2. Mod Month View (Persis Imej Rujukan Anda)
function renderMonthView() {
    ...
    // Sembunyi / Tunjuk kolum hujung minggu
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const dayOfWeek = dayDateObj.getDay(); // 0 = Ahad, 6 = Sabtu
    ...
    // Kemas kini PnL Bulan & Monthly Goal Bar
...
function renderAllTimeView() {
    ...
    if (sortedYears.length === 0) sortedYears.push('2026');
...
// ================= TIMEFRAME ANALYTICS =================
function openTimeframeModal() { document.getElementById('timeframeModal').classList.remove('opacity-0', 'pointer-events-none'); }
function closeTimeframeModal() { document.getElementById('timeframeModal').classList.add('opacity-0', 'pointer-events-none'); }
function selectAnalyticsTimeframe(tf) { analyticsTimeframe = tf; document.getElementById('analyticsTimeframeText').textContent = tf; closeTimeframeModal(); renderAnalytics(); showToast(`Julat: ${tf}`); }
...
function goToToday() { currentDate = new Date(); renderMonthView(); showToast('Hari ini'); }

function openIntradayView(dateStr) {
    selectedDayStr = dateStr;
    document.getElementById('calView-month').classList.add('hidden');
    document.querySelector('.grid.grid-cols-4.gap-1').classList.add('hidden'); // Sembunyi tab atas
